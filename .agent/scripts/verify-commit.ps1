...
$ADRFile = ".agent/memory/architecture-decisions.md"
...
if (Test-Path $ADRFile) {
    ...
    if ($null -eq $Content -or $Content.Count -eq 0 -or ($Content.Count -eq 1 -and $Content -match "^# Architecture Decision Records")) {
