...
### Beads is not found
- **Linux/macOS:** Install Go first, then run `go install github.com/steveyegge/beads@latest`. Initialize with `bd init` in your project directory.
- **Windows:** Use `npm install -g @beads/bd`. If that fails, download the binary directly from https://github.com/steveyegge/beads/releases and place `bd.exe` in your PATH.
